# PRD: Neo4j Graph Connector — Graph Sink Data Products

## Problem Statement

Ontos manages Data Products backed by Databricks Unity Catalog assets (tables, views, models). **Graph databases are an important and underserved product type**: they are populated by ETL engines that pull from Databricks and build relationship-rich graph representations of operational data, yet Ontos has no way to register or govern them. Specifically:

1. **No product type for graph databases.** The ODPS `productType` vocabulary covers `source`, `source-aligned`, `aggregate`, `consumer-aligned`, and `sink`, but not Neo4j graph sinks. There is no way for a Data Producer to register a Neo4j database as a first-class data product.
2. **No live graph metadata surface.** Even if a product is registered, there is no way to inspect what node labels, relationship types, node counts, or data freshness indicators exist inside the graph without leaving Ontos and connecting directly to Neo4j.
...
5. **No catalog representation of graph entities.** The node labels inside a Neo4j graph are logical datasets with properties. They are invisible to Ontos's asset catalog, making governance and lineage impossible.

## Solution

### `sink-graph-neo4j` Product Type

A new product type value `sink-graph-neo4j` (displayed as **"Sink - Graph (Neo4j)"**) is added to the ODPS `productType` vocabulary in Ontos. Selecting it in the create/edit dialog reveals a dedicated connection configuration section for the Neo4j instance. The panel on the product details page is gated on this type — no other product type renders the graph UI.
...
All endpoints accept `bolt_url`, `username`, `database`, `secret_scope`, and `secret_key` as query parameters. The `summary` endpoint is the one called by the frontend panel. The handler (`Neo4jDeliveryHandler`) opens a Bolt session, fires read-only Cypher queries (node counts, relationship counts, topology), optionally reads a configurable freshness tracker node type, and closes the connection on every request — no persistent connection pool.
...
### Neo4j Graph Management Port Convention

A `sink-graph-neo4j` product's Neo4j connection is stored as a management port with `content: "observability"`. The `url` field holds the **Bolt URL** of the Neo4j instance. The frontend panel reads connection parameters from the port's `customProperties` and invokes the backend summary endpoint by **POST with a JSON body** — never as GET query parameters — so that the Bolt URL, username, secret scope, and secret key are never written to server access logs, proxy logs, or browser history. The port is identified explicitly by the `sink-graph-neo4j` product type gate rather than by URL heuristics.

### Cytoscape.js Graph Visualization Panel

A new frontend component, `Neo4jGraphPanel`, renders the graph topology as an interactive node-link diagram using **Cytoscape.js** (already installed in the project as `react-cytoscapejs`). It replaces a prior tabular display and matches the visual language of the Concepts dashboard's graph view.
...
The panel only mounts and fetches when the product type is `sink-graph-neo4j` and a management port with a URL is present; it returns `null` for all other products.
...
1. As a **Data Producer**, I want to select "Sink - Graph (Neo4j)" as my product type when creating a new data product, so that I can register a Neo4j database as a first-class Ontos product.
...
3. As a **Data Producer**, I want the Neo4j connection fields to only appear when I select "Sink - Graph (Neo4j)" as the product type, so that the form is not cluttered for other product types.
4. As a **Data Consumer**, I want to see a live interactive graph topology on a Sink - Graph (Neo4j) product details page, so that I can understand the schema structure without leaving Ontos.
...
8. As a **Data Consumer**, I want to see the data freshness per feed alongside the graph, so that I can assess the timeliness of the product.
...
14. As a **Data Steward**, I want Neo4j node labels to appear as Dataset assets in the Asset Explorer with their logical attributes, so that I can apply tags, quality rules, and governance policies to graph entities.
15. As a **Data Consumer**, I want the graph panel to show a clear error message if the Neo4j instance is unreachable, so that I know the product has a connectivity issue rather than seeing a crash.
...
```
Frontend panel
  → reads neo4jPort.url (Bolt URL), customProperties[username, database, secret_scope, secret_key]
  → POST /api/neo4j/summary  { bolt_url, username, database, secret_scope, secret_key }
     (JSON body — never query parameters, so nothing appears in access logs or browser history)
Backend route
...
No password is logged, persisted, or returned in any API response. The secret scope and key references are treated with the same confidentiality as the password itself.
...
- **`TestNeo4jEndpoints`**: Correctness of all four API endpoints. Asserts on specific node labels and relationship types from the demo graph. Asserts that unreachable Bolt URLs return `connected: false` (not HTTP 500).
- **`TestDataProductReadiness`**: Verifies that the demo Neo4j graph product passes ODPS metadata, output port contract, logical attribute, and business lineage readiness checks.
...
- **Multiple Neo4j instances per product** — a `sink-graph-neo4j` product has exactly one management port with a Bolt URL. Supporting multiple instances (e.g., one per environment) is deferred.
...
2. **Product type + connection form** — `sink-graph-neo4j` type in the create dialog, Neo4j connection fields, management port storage convention.
...
- **Cytoscape.js (Concepts dashboard)**: The graph panel reuses `react-cytoscapejs` and `cytoscape`, already installed for the Concepts dashboard's graph visualization component (`knowledge-graph.tsx`). The stylesheet and layout API are deliberately consistent with that component so the two graph surfaces feel like one visual system.
